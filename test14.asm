...
_test14:     file format elf32-i386
...
main(int argc,char *argv[])
{
  ...
  18:	83 ec 0c             	sub    $0xc,%esp
	...
	if (pid || !pid)
	{
		...
		{
			*key += i+2;
  1b:	8d 43 33             	lea    0x33(%ebx),%eax
			addr[i] = shmget(key);
  1e:	68 40 07 00 00       	push   $0x740
	//pid = fork(); 		//if i allow this , max process per page error will occur
	if (pid || !pid)
	{
		for (i=0;i<n;i++)
		{
			*key += i+2;
  23:	a2 40 07 00 00       	mov    %al,0x740
			addr[i] = shmget(key);
  28:	e8 15 03 00 00       	call   342 <shmget>
			addr[i][0] = 'a';
  2d:	c6 00 61             	movb   $0x61,(%eax)
			printf(1,"%d-%x , key:%s\n",i,(unsigned int) addr[i],key);
  30:	c7 04 24 40 07 00 00 	movl   $0x740,(%esp)
  37:	50                   	push   %eax
  38:	53                   	push   %ebx
	// pid = fork();
	// pid = fork();
  ...
  39:	83 c3 01             	add    $0x1,%ebx
		{
			*key += i+2;
			...
			addr[i][0] = 'a';
			printf(1,"%d-%x , key:%s\n",i,(unsigned int) addr[i],key);
  3c:	68 46 07 00 00       	push   $0x746
  41:	6a 01                	push   $0x1
  43:	e8 d8 03 00 00       	call   420 <printf>
			sleep(5);
  48:	83 c4 14             	add    $0x14,%esp
  4b:	6a 05                	push   $0x5
  4d:	e8 e0 02 00 00       	call   332 <sleep>
	// pid = fork();
	// pid = fork();
	//pid = fork(); 		//if i allow this , max process per page error will occur
	if (pid || !pid)
  ...
  55:	83 fb 20             	cmp    $0x20,%ebx
  58:	75 be                	jne    18 <main+0x18>
			printf(1,"%d-%x , key:%s\n",i,(unsigned int) addr[i],key);
			sleep(5);
		}
	}
	...
	exit();	
  5a:	e8 43 02 00 00       	call   2a2 <exit>
  5f:	90                   	nop
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3bf:	f7 f6                	div    %esi
 3c1:	0f b6 92 60 07 00 00 	movzbl 0x760(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 538:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 53b:	b8 56 07 00 00       	mov    $0x756,%eax
...
free(void *ap)
{
 ...
 617:	89 08                	mov    %ecx,(%eax)
  freep = p;
 619:	a3 fc 09 00 00       	mov    %eax,0x9fc
}
 61e:	5b                   	pop    %ebx
 61f:	5e                   	pop    %esi
 620:	5f                   	pop    %edi
 ...
 63a:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 63d:	a3 fc 09 00 00       	mov    %eax,0x9fc
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 6af:	76 3f                	jbe    6f0 <malloc+0xa0>
    ...
    if(p == freep)
 6b1:	39 05 fc 09 00 00    	cmp    %eax,0x9fc
 ...
 6d6:	e8 e5 fe ff ff       	call   5c0 <free>
  return freep;
 6db:	8b 15 fc 09 00 00    	mov    0x9fc,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
 ...
 6fc:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
 6ff:	89 15 fc 09 00 00    	mov    %edx,0x9fc
      return (void*)(p + 1);
 ...
 714:	eb e9                	jmp    6ff <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 716:	c7 05 fc 09 00 00 00 	movl   $0xa00,0x9fc
 71d:	0a 00 00 
 720:	c7 05 00 0a 00 00 00 	movl   $0xa00,0xa00
 727:	0a 00 00 
    base.s.size = 0;
 72a:	b8 00 0a 00 00       	mov    $0xa00,%eax
 72f:	c7 05 04 0a 00 00 00 	movl   $0x0,0xa04
 736:	00 00 00 
